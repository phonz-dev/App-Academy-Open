...
def factors(num)
  all_nums = (1..num).to_a
  all_nums.select { |fact| num % fact == 0 }
end
...
class Array
  ...
  def bubble_sort(&prc)
    duplicate = self.dup
    duplicate.bubble_sort! { |num1, num2| prc.call(num1, num2) }
  end
end
...
def substrings(string)
  ...
  (0...string.length).each do |start_pos|
    (start_pos...string.length).each do |end_pos|
      all_substrings << string[start_pos..end_pos]
    end
  end
  all_substrings
end
...
def subwords(word, dictionary)
  ...
  word_substrings.select do |substring|
...
class Array
  ...
  def my_inject(&blk)
    duplicate = self.dup
    outcome = duplicate.shift
    duplicate.my_each do |el|
      outcome = blk.call(outcome, el)
    end
    outcome
  end
end
